...
public class BurpExtender implements IBurpExtender, IExtensionStateListener {
    ...
    public void registerExtenderCallbacks(IBurpExtenderCallbacks callbacks) {
        ...
        File tmpFile = new File(extensionPath, "fingerPrint-tmp.json");
        InputStream inputStream = null;
        if (tmpFile.exists()) {
            try {
                inputStream = new FileInputStream(tmpFile);
            } catch (FileNotFoundException e) {
                stderr.println("[!] Failed to load the configuration file finger.json, because fingerPrint-tmp.json not found");
                return;
            }
        } else {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream, StandardCharsets.UTF_8))) {
            ...
            if (tmpFile.exists()) {
                stdout.println("[+] Successfully loaded the configuration file fingerPrint-tmp.json");
            }else{
